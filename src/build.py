...
def get_text_from_thread_id(reddit_thread_id, prawler, reddit_id_to_text_mapping):
    ...
    return reddit_id_to_text_mapping.get("reddit_thread_id")


def get_fun_facts_text_from_thread_id(
    reading_set, key, agent, fact_section_key, prawler, reddit_id_to_text_mapping
):
...
def get_fun_facts_text_and_wiki_text_for_all_fact_sections(
    reading_set, key, agent, prawler, reddit_id_to_text_mapping, wikidata
):
    fact_section_keys = ["FS1", "FS2", "FS3"]
    for fact_section_key in fact_section_keys:
        reading_set[key][agent][fact_section_key][
            "fun_facts"
        ] = get_fun_facts_text_from_thread_id(
            reading_set,
            key,
            agent,
            fact_section_key,
        ...
        if "shortened_wiki_lead_section" in reading_set[key][agent][fact_section_key]:
            wiki_id = reading_set[key][agent][fact_section_key][
                "shortened_wiki_lead_section"
            ]
            reading_set[key][agent][fact_section_key][
                "shortened_wiki_lead_section"
            ] = get_wiki_text("shortened_wiki_lead_section", wiki_id, wikidata)
        ...
        if "summarized_wiki_lead_section" in reading_set[key][agent][fact_section_key]:
            ...
            reading_set[key][agent][fact_section_key][
                "summarized_wiki_lead_section"
            ] = get_wiki_text("summarized_wiki_lead_section", wiki_id, wikidata)

    return reading_set[key][agent]


def create_post_built_reading_set_file(
    input_directory,
    output_directory,
    filename,
    reddit_id_to_text_mapping,
        ...
        for key in tqdm(reading_set.keys()):
            ...
            if article_url is not None and reading_set[key]["config"] is not "C":
                if article_url not in article_url_to_content_mapping.keys():
                    topical_content_wapo_section = (
                        wapo_retriever.get_wapo_article_section(article_url)
                    )
                    article_url_to_content_mapping[
                        article_url
                    ] = topical_content_wapo_section
                else:
                    topical_content_wapo_section = article_url_to_content_mapping[
                        article_url
                    ]
            ...
            for agent in agent_keys:
                reading_set[key][
                    agent
                ] = get_fun_facts_text_and_wiki_text_for_all_fact_sections(
                    reading_set,
                    key,
                    agent,
                    prawler,
...
def verify(reading_set_post_build_folder):
    hash_file_path = os.path.join(
        reading_set_post_build_folder, "post_build_file_hashes.json"
    )
    generated_post_build_reading_set_files = [
        "test_freq.json",
        "test_rare.json",
        "train.json",
        "valid_freq.json",
    ...
    with open(hash_file_path, "r") as post_build_hashes_file:
        ...
        for file_name in generated_post_build_reading_set_files:
            post_build_file_path = os.path.join(
                reading_set_post_build_folder, file_name
            )
...
def main():
    ...
    parser.add_argument(
        "--reddit_client_id",
        help="You can obtain your client ID "
        "and client secret by signing "
        "up for Reddit credentials here: https://www.reddit.com",
        dest="CLIENT_ID",
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--reddit_user_agent",
        help="Please supply any user agent",
        dest="USER_AGENT",
        required=True,
    )
    ...
    reddit_id_to_text_mapping = loaded
    article_url_to_content_mapping = {}
...
#     for filename in ["test_freq.json"]:
        ...
        create_post_built_reading_set_file(
            ...
            reading_set_post_build_directory,
            filename,
            reddit_id_to_text_mapping,
            article_url_to_content_mapping,
            prawler,
